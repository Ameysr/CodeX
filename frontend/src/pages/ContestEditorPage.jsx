...
const ContestEditorPage = () => {
  ...
  // 1. Effect for fetching contest and problem data (only when contestId or problemId changes)
  useEffect(() => {
  ...
  }, [contestId, problemId]); // Remove selectedLanguage from dependencies
  ...
  useEffect(() => {
    ...
    if (savedCode) {
      ...
      if (initialCodeObj) {
        ...
        console.error(`Initial code not found for language: ${selectedLanguage}`);
        setCode("");
      }
    }
  }, [problem, selectedLanguage, contestCodeStore, contestId, problemId]);
  ...
  // Single handleLanguageChange function
