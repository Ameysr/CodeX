...
const ProblemPage = () => {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden" style={{ backgroundColor: "#181C1F" }}>
      {/* Funding Banner */}
      {showFundingBanner && (
        <div className="text-white px-6 py-2 flex items-center justify-between relative shadow-lg z-[60]" style={{ backgroundColor: "#4C99EF" }}>
          <div className="flex-1 flex items-center justify-center font-medium text-sm md:text-base">
            <span className="flex items-center gap-2">
              <span>📢 The free tier of the code execution judge0 api is finished fund me to run your code</span>
            </span>
          </div>
          <button
            onClick={() => setShowFundingBanner(false)}
      ...
      <div
        className={`fixed inset-0 z-50 flex transition-opacity duration-300 ease-in-out ${showProblemList ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
          }`}
      >
        <div
          className={`absolute inset-0 bg-black transition-opacity duration-300 ${showProblemList ? 'opacity-50' : 'opacity-0'
            }`}
          onClick={toggleProblemList}
        ></div>

        <div
          className={`relative z-50 w-80 bg-gray-900 shadow-xl overflow-y-auto transform transition-transform duration-300 ease-in-out ${showProblemList ? 'translate-x-0' : '-translate-x-full'
            }`}
            ...
            {sidebarProblems.length === 0 ? (
              ...
              <div className="space-y-1">
                {filteredSidebarProblems.map((p) => (
                  <button
                    key={p._id}
                    className={`w-full text-left p-3 rounded-lg transition-all ${p._id === problemId
      ...
      >
        ...
        <div className="flex gap-3">
          ...
          >
            <Clock size={16} />
            <span
              className={`ml-1 inline-block transition-all duration-300 ${timerActive ? "opacity-100 w-auto" : "opacity-0 w-0 overflow-hidden"
                }`}
            >
              {formatTime(timer)}
            </span>
